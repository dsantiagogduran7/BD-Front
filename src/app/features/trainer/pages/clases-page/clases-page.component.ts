...
@Component({
  selector: 'app-trainer-clases-page',
  standalone: true,
  imports: [CommonModule, FormsModule, PaginacionComponent],
  templateUrl: './clases-page.component.html',
  styleUrl: './clases-page.component.css'
})
export class TrainerClasesPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      clases:   this.clasesApi.listarPorEntrenador(this.myCedula),
      deportes: this.deportesApi.listarTodos(),
      salas:    this.salasApi.listarTodas()
    }).subscribe({
      next: ({ clases, deportes, salas }) => {
  ...
  get clasesFiltradas(): ClaseDto[] {
    return this.clases.filter(c => {
      const txt = (c.deporte?.nombre ?? '').toLowerCase();
      const ok1 = !this.busqueda     || txt.includes(this.busqueda.toLowerCase());
      const ok2 = !this.filtroEstado || c.estado === this.filtroEstado;
      return ok1 && ok2;
    });
  }
  get totalPaginas(): number { return Math.max(1, Math.ceil(this.clasesFiltradas.length / this.itemsPorPagina)); }
  get clasesPaginadas(): ClaseDto[] {
    const i = (this.paginaActual - 1) * this.itemsPorPagina;
    return this.clasesFiltradas.slice(i, i + this.itemsPorPagina);
  }
  cambiarPagina(n: number): void { this.paginaActual = n; }

  onFechaChange(): void {
    if (!this.form.fecha) { this.horariosFiltrados = []; this.form.id_horario = 0; return; }
    const dia = DIA_MAP[new Date(this.form.fecha + 'T00:00:00').getDay()];
    this.form.id_horario = 0;
    this.horariosApi.listarPorDia(dia).subscribe({
  ...
  activarEdicion(): void {
    ...
    this.form = {
      ...
      cedula_entrenador: this.myCedula
    };
    if (c.fecha) {
      const dia = DIA_MAP[new Date(c.fecha + 'T00:00:00').getDay()];
      this.horariosApi.listarPorDia(dia).subscribe({
        next: h  => { this.horariosFiltrados = h; },
        error: () => { this.horariosFiltrados = []; }
      });
  ...
  guardar(): void {
    ...
    obs.subscribe({
      ...
      error: (err) => { this.errorValidacion = err?.error?.error ?? 'Error al guardar la clase.'; }
    });
  }
  ...
  private recargar(): void {
    this.clasesApi.listarPorEntrenador(this.myCedula).subscribe({
  ...
  private formVacio(): ClaseFormDto {
    return {
      ...
      id_deporte: 0, cedula_entrenador: this.myCedula
    };
  }
  ...
  private formValido(): boolean {
    ...
    if (!f.comentario.trim())      { this.errorValidacion = 'El comentario es obligatorio.';       return false; }
    return true;
  }
}
